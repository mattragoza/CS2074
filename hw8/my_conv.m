function [output] = my_conv(input, filter, pad, stride)
    debug = false;

    % input and filter size
    [H_in, W_in] = size(input);
    [H_ft, W_ft] = size(filter);
    ...
    if true
        fprintf( ...
            'my_conv([%d x %d], size=%d, pad=%d, stride=%d) -> [%d x %d]\n', ...
    ...
    for i=1:H_out
        for j=1:W_out
            ...
            % compute dot product of window and filter
            output(i,j) = sum(filter .* window, 'all');
        end
    end
end